...
function readAsunData(input, out, type, maxValues) {
    ...
    var index = out.length, start= out.length;
    for (var i = 0; i < input.length; i++) {
        if (input[i].code != globals.currentCity) {
             continue;
        }
        var item = {
            type: type,
...
//	            balance: parseFloat(input[i]["17"] || 0)
    ...
    console.log("asunData", out.length-start);
}
...
//	            balance: parseFloat(input[i]["17"] || 0)
    ...
    console.log("flatData", out.length-start);
}
...
//    initialInputDataProfile= [];
//    window.initialInputDataFlats= [];
//    window.initialInputDataASUN= [];
    return result;
};
...
function calculateMaxCells(inputData) {
    ...
    var initX= globals.cityCenters[globals.currentCity].x;
    var initY= globals.cityCenters[globals.currentCity].y;
